class Solution:
    ...
    def sortItems(self, n: int, m: int, group: List[int],
        ...
        def ts(nodes, pred, succ):
            order, no_pred = [], deque(node for node in nodes
                                       if not pred[node])
            while no_pred:
        ...
        groups_order = ts(list(group_to_items.keys()), inter_pred, inter_succ)
        if not groups_order:
            return []

        items_order = []
        for grp in groups_order:
            order = ts(group_to_items[grp], intra_pred, intra_succ)
            if not order:
                return []
