class Solution:
    ...
    def minExtraChar(self, s: str, dictionary: List[str]) -> int:
        ...
        dp = [max_len] * (
            max_len
        )  # Initialize an array to store minimum extra characters needed for substrings

        dp[0] = 0  # No extra characters needed for an empty string
        word_set = set(
            dictionary
        )  # Convert the dictionary into a set for efficient word look-up

        for i in range(1, max_len):
            dp[i] = (
                dp[i - 1] + 1
            )  # Initialize with a worst-case scenario (one more character than previous)
        ...
        return dp[
            -1]  # Return the minimum extra characters needed for the entire string
